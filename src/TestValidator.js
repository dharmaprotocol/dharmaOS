...
class TestValidator {
	...
	parseActionScriptTestByPath([name, filepath]) {
		...
		if (!('tests' in parsed && TestValidator.TYPE_CHECKERS.array(parsed.tests))) {
			throw new Error(
		...
		for (let test of parsed.tests) {
			for (let [field, defaultValue] of Object.entries(TestValidator.TEST_LEVEL_DEFAULTS)) {
	...
	validateActionScriptTests() {
		...
		const actionScriptNames = new Set(this.actionScripts.map(script => script.name));
		const actionScriptTestNames = new Set(this.actionScriptTests.map(test => test.name));
	...
	validateActionScriptTest(actionScriptTest) {
		for (let [field, type] of Object.entries(TestValidator.TOP_LEVEL_FIELDS_AND_TYPES)) {
		...
		const actionScriptOptions = this.actionScripts.filter(script => script.name === name);
		...
		for (let [testNumber, test] of Object.entries(tests)) {
			if (!test || !('name' in test)) {
				throw new Error(
					`Action script test "${name}" test #${parseInt(testNumber) + 1} does not itself have a "name" field`
				);
			}
			const testName = test.name;

			for (let [field, type] of Object.entries(TestValidator.TEST_LEVEL_FIELDS_AND_TYPES)) {
				if (!(field in test)) {
					throw new Error(
			...
			for (const [variableName, variableType] of Object.entries(actionScript.variables)) {
				...
				if (variableType in TestValidator.VARIABLE_TYPE_CHECKERS) {
					if (!TestValidator.VARIABLE_TYPE_CHECKERS[variableType](test.variables[variableName])) {
						throw new Error(
			...
			for (const [resultName, resultType] of Object.entries(actionScript.results)) {
				...
				if (resultType in TestValidator.VARIABLE_TYPE_CHECKERS) {
					if (!TestValidator.VARIABLE_TYPE_CHECKERS[resultType](test.results[resultName])) {
						throw new Error(
			...
			for (const [eventIndex, event] of Object.entries(test.events)) {
				for (let [field, type] of Object.entries(TestValidator.EVENT_LEVEL_FIELDS_AND_TYPES)) {
					if (!(field in event)) {
						throw new Error(
							`Action script test "${testName}" on "${name}" event #${parseInt(eventIndex) + 1} must contain a "${field}" field`
						);
					}
					if (!TestValidator.TYPE_CHECKERS[type](event[field])) {
						throw new Error(
							`Action script test "${testName}" on "${name}" event #${parseInt(eventIndex) + 1} has a field "${field}" that is not of type "${type}"`
						);
					}
				}
